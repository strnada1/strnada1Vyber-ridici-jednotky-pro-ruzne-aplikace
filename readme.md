...
| :----------------------- | :------------------------------------ | :--------------------------------------------------------------------------------------------------- | :------------------------------------------ | :--------------------------------------- |
| **MCU**                  | Micro controller | Integrovaný čip (CPU + RAM + Flash na jednom substrátu), deterministický běh bez OS nebo RTOS | např. ESP32, PIC16LF1xxx, RP2040  | Kalkulačka, ABS na autech |
| **MPU**                  | Micro Processor | Samostatný procesor vyžadující externí RAM a úložiště, zpravidla běží plnohodnotný OS (Linux) |  Raspberry pi, Microchip SAMA5D2 | PC, mobil, NAS servery |
| **Embedded**             | Vestavený systém | jednoúčelový počítačový systém, který je pevně součástí většího zařízení a plní v něm konkrétní, předem určený úkol | Embedded PLC, Embedded | Bílá technika, bankomaty, regulace kotlů |
| **PLC**                  | Programmable logic controller | Průmyslový automat pro cyklické deterministické řízení procesů, vysoká odolnost, modulární/kompaktní |  Allen-Bradley Micro800, Omron Sysmac NX1 | Řízení semaforů, montážních linel |
| **iPC**                  | industrial/průmyslový PC | běžný PC upravený pro neustálý provoz a odolný vůči prachu vibracím a vysokým teplotám |  Advantech UNO 2000, Beckhoff C6015 | Velín a dispečink v pivovaru nebo teplárně |
| **Programovatelné relé** | Programovatelné relé | Zjednodušené kompaktní PLC pro méně náročné úlohy, nahrazuje časovací relé a stykačové kombinace | rodina Siemens LOGO! | domácí automatizace, řízení čerpadel |
...
| :---------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------------ | :------------------------ | :----------------------------------- |
| **Doporučená platforma** *(MCU / PLC / iPC)*                                                          | **Programovatelné relé / kompaktní PLC** *(např. Siemens LOGO!, Eaton easyE4)*                                                                                                                                              | `MCU - ESP32-DevKitC V4`      | `PLC - Siemens SIMATIC S7-1200 (CPU 1214C)` | `iPC - Advantech ARK-3534B`|
...
| **Primární programovací jazyk** *(C/C++/MicroPython vs. IEC 61131-3 ST/LAD vs. Python/C#/C++ pod OS)* | **FBD / LAD** *(grafické funkční bloky nebo liniové schéma dle IEC 61131-3)*                                                                                                                                                | ` micropython `                        | `ST/LAD`           | `C++/Python`               |
| **Klíčový technický argument pro volbu** *(např. spotřeba, determinismus, grafický výkon)*            | Montáž přímo na DIN lištu v rozváděči, integrovaný displej pro nastavení časovačů přímo na místě, robustní reléové výstupy pro motor a semafor, napájení 24 V DC / 230 V AC bez nutnosti vývoje vlastního plošného spoje.  | `jednoduchá instalace, velký výběr, možnost propojení s mobilem a chytrou domácností` | `potřebný výkon pro počítání, spolehlivost 24/7 jednoduchá úprava programu`                     | `...`                                |
| **Hlavní riziko při volbě špatné platformy** *(proč by neuspěly ostatní dvě varianty)*                | **MCU:** Nutnost vývoje vlastní desky, nízká odolnost vůči venkovnímu rušení a obtížný servis údržbou.<br>**iPC:** Zbytečně extrémní cena (> 30 tis. Kč), dlouhý start po výpadku napájení a vysoká spotřeba.              | `vysoká cena, nepůjde napájet z baterky, komplikovaný programování ` | `nedostatečný výkon, potřeba častého servisu, pro IPC vysoká cena `                     | `...`                                |
